...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <link rel="stylesheet" href="{{ asset('landingpage/css/skin.css') }}" />
                ...
                <div id="section" class="box">
                    <div id="content">
                        <ul class="articles box">
                            <li>
                                <h2><a href="#">{{ $berita->judul_berita }}</a></h2>
                                <div class="article-info box">
                                    <p class="f-left">Diposting pada {{ $berita->created_at->isoFormat('D MMMM Y') }} | {{ $berita->kategori->nama_kategori }} | haji.kemenag.go.id</p>
                                </div>
                                <p><img src="{{ asset('/uploads/'. $berita->gambar_berita) }}" alt="" class="f-left" style="width:400px; height:250px;"/>{!! $berita->isi_berita !!}</p>
                            </li>
                        </ul>
                    </div>
                    <div id="aside">
                        <h3 class="nomb">Berita Lainnya</h3>
                        @foreach ($beritaLainnya as $row)
                            <ul class="sponsors">
                                <li>
                                    <img src="{{ asset('/uploads/'. $row->gambar_berita) }}" alt="" class="f-left" style="width:100px; height:50px;"/>
                                    <a href="{{ route('detail-berita', $row->slug) }}">{{ $row->judul_berita }}</a><br/>
                                    {{ $row->created_at->isoFormat('D MMM Y') }}
                                </li>
                            </ul>
                        @endforeach
                    </div>
